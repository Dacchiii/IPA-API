(() => {
 ...
 const cleanStr = str.replace(/\0+$/, ''); // 念のため終了バイト除去
 return [...cleanStr].slice(-1)[0]; // 配列壊さずに最後の文字取得
 }
 ...
 function pickDiacritic(f, lastBuffer){
  let out = "";

  // 長音
  if(f.volume>0.2){
   const last = lastChar(lastBuffer);
   if(last !== "ː") out += "ː";
  }

  // 鼻音化
  if(lastChar(lastBuffer) === "a" || lastChar(lastBuffer) === "o" || lastChar(lastBuffer) === "e"){
   if(lastChar(lastBuffer) !== "̃") out += "̃";
  }

  // 母音無声化
  if(f.zcr > 4000){
   if(lastChar(lastBuffer) !== "̥") out += "̥";
  }
